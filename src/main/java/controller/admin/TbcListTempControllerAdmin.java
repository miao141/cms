...
@Controller
@RequestMapping("/admin/TbcListTemp/")
public class TbcListTempControllerAdmin extends BaseController{	private final static Logger log= Logger.getLogger(TbcListTempControllerAdmin.class);
	...
	private TbcListTempService<TbcListTempModel> tbcListTempService; 
	...
	public ModelAndView index(String id, ModelMap modelMap, HttpServletRequest request) {
		...
		return new ModelAndView("admin/TbcListTemp/index", modelMap);
	}
	...
	public String charts(){
		return "admin/TbcListTemp/charts";
	}
	...
	public void chartsJson(HttpServletResponse response,String startTime,String endTime,String chartsName,Integer type) {
		...
		try {
			List<Map<?, ?>> list = tbcListTempService.charts(paramMap);
	...
	public String baseDlg(){
		return "admin/TbcListTemp/baseDlg";
	}
	...
	public String importDlg(){
		return "admin/TbcListTemp/importDlg";
	}
	...
	public String searchDlg(){
		return "admin/TbcListTemp/searchDlg";
	}
   ...
   /**
     ...
     * <b>日期：</b> 2013-7-31 <br>
     * @param pageParams
     * @param tbcListTempModel
     * @return
     * @throws Exception
     */
	@RequestMapping("data.html")
	@ResponseBody
	public String data(PageParams pageParams, TbcListTempModel tbcListTempModel) throws Exception {
		System.out.println("pageParams:" + pageParams + "|tbcListTempModel:" + tbcListTempModel);
		tbcListTempModel.getPageUtil().setPaging(true);
		String result = "[]";
		if (pageParams.getPage() != null) {
			try {
				tbcListTempModel.getPageUtil().setPageId(Integer.parseInt(pageParams.getPage())); // 当前页
			} catch (Exception e) {
				log.error(e);
			}
		}
		if (pageParams.getRows() != null) {
			try {
				tbcListTempModel.getPageUtil().setPageSize(Integer.parseInt(pageParams.getRows()));// 显示X条
			} catch (Exception e) {
				log.error(e);
			}
		}
		if (pageParams.getSort() != null) {
			try {
				tbcListTempModel.getPageUtil().setOrderByCondition(pageParams.getSort()); // 排序字段名称
			} catch (Exception e) {
				log.error(e);
			}
		}

		// tbcListTempModel.getPageUtil().setOrderDirection(true); //true false 表示 正序与倒序
        String str="";
        String suffix = "}";
		...
		List<TbcListTempModel> listTbcListTempModel = null;
		StringBuilder center = new StringBuilder();

		if (pageParams.getSearchType() != null) {
			if (pageParams.getSearchType().equals("1")) { // 模糊搜索
				tbcListTempModel.getPageUtil().setLike(true);
				listTbcListTempModel = tbcListTempService.selectByModel(tbcListTempModel);
				center.append("{\"total\":\"" + tbcListTempModel.getPageUtil().getRowCount() + "\",\"rows\":"+str);
			} else {
				...
				if (sb.length() > 0) {
					...
					tbcListTempModel.getPageUtil().setAndCondition(sb.toString());
					listTbcListTempModel = tbcListTempService.selectByModel(tbcListTempModel);
					center.append("{\"total\":\"" + tbcListTempModel.getPageUtil().getRowCount() + "\",\"rows\":"+str);
				}
			}
			...
			if (pageParams.getGridName() == null) {
				listTbcListTempModel = tbcListTempService.selectByModel(tbcListTempModel);
				center.append("{\"total\":\"" + tbcListTempModel.getPageUtil().getRowCount() + "\",\"rows\":");
				suffix = "}";
			} else {
			}
		}
		...
		if (pageParams.getGridName() == null) { //datagrid
			center.append(JSON.toJSONString(listTbcListTempModel));
		} else {
		}
	...
	public void tbcListTempExport(HttpServletResponse response,TbcListTempModel tbcListTempModel) {
		List<TbcListTempModel> listTbcListTempModel=null;
		try {
			listTbcListTempModel=tbcListTempService.selectByModel(tbcListTempModel);
		} catch (Exception e1) {
		...
		Map<String, List<TbcListTempModel>> exportMap = new HashMap<String, List<TbcListTempModel>>();
		exportMap.put("sheet", listTbcListTempModel);
	...
	public void tbcListTempImport(HttpServletRequest request, HttpServletResponse response){
		...
		for (Map.Entry<String, MultipartFile> entity : fileMap.entrySet()) {
			...
			try {
				Map<String, List<TbcListTempModel>> ml=excelUtil.readExcel(new ByteArrayInputStream(mf.getBytes()), columnName, 2, TbcListTempModel.class);
				for(Map.Entry<String, List<TbcListTempModel>> map:ml.entrySet()){
					List<TbcListTempModel> lt=map.getValue();
					for(int i=0;i<lt.size();i++){
						try {
							tbcListTempService.insert(lt.get(i));
						} catch (Exception e) {
							e.printStackTrace();
						}
					}
     ...
     * 
     * <br>
     * <b>功能：</b>增加 TbcListTempModel<br>
     * <b>作者：</b> <br>
     * <b>日期：</b> 2012-10-11 <br>
     * @return
     */
	@RequestMapping("add.html")
	public void add(TbcListTempModel tbcListTempModel,HttpServletResponse response){
		tbcListTempModel.setId(util.getUid());//设置主键
		System.out.println("tbcListTempModel:"+tbcListTempModel.toString());
	    try {
			if(tbcListTempService.insert(tbcListTempModel)>0){
				util.toJsonMsg(response, 0, null);
				return;
			};
		} catch (Exception e) {
     ...
     * 
     * <br>
     * <b>功能：</b>保存 TbcListTempModel<br>
     * <b>作者：</b> <br>
     * <b>日期：</b> 2012-10-11 <br>
     * @return
     */
	@RequestMapping("save.html") 
	public void save(TbcListTempModel tbcListTempModel,HttpServletResponse response){
		try{
		 	if(tbcListTempService.updateByPrimaryKey(tbcListTempModel)>0){
			    util.toJsonMsg(response, 0, null);
			    return;
			 }
		}catch(Exception e){
	...
	public void del(String[] ids,HttpServletResponse response){
		...
		try{
			 if(null!=ids && ids.length>0){
				 if(tbcListTempService.deleteByPrimaryKeys(ids)>0){
					 util.toJsonMsg(response, 0, null);
					 return;
				 }
			 }

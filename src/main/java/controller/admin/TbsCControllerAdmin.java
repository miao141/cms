...
@Controller
@RequestMapping("/admin/TbsC/")
public class TbsCControllerAdmin extends BaseController{	private final static Logger log= Logger.getLogger(TbsCControllerAdmin.class);
	...
	private TbsCService<TbsCModel> tbsCService; 
	...
	public ModelAndView index(String id, ModelMap modelMap, HttpServletRequest request) {
		...
		return new ModelAndView("admin/TbsC/index", modelMap);
	}
	...
	public void chartsJson(HttpServletResponse response,String startTime,String endTime,String chartsName,Integer type) {
		...
		try {
			List<Map<?, ?>> list = tbsCService.charts(paramMap);
   ...
   /**
	...
	@ResponseBody
	public String data(PageParams pageParams, TbsCModel tbsCModel) throws Exception {
		System.out.println("pageParams:" + pageParams + "|tbsCModel:" + tbsCModel);
		tbsCModel.getPageUtil().setPaging(true);
		String result = "[]";
		if (pageParams.getPage() != null) {
			try {
				tbsCModel.getPageUtil().setPageId(Integer.parseInt(pageParams.getPage())); // 当前页
			} catch (Exception e) {
				log.error(e);
			}
		}
		if (pageParams.getRows() != null) {
			try {
				tbsCModel.getPageUtil().setPageSize(Integer.parseInt(pageParams.getRows()));// 显示X条
			} catch (Exception e) {
				log.error(e);
			}
		}
		if (pageParams.getSort() != null) {
			try {
				tbsCModel.getPageUtil().setOrderByCondition(pageParams.getSort()); // 排序字段名称
			} catch (Exception e) {
				log.error(e);
			}
		}

		// tbsCModel.getPageUtil().setOrderDirection(true); //true false 表示 正序与倒序
        String str="";
        String suffix = "}";
		...
		List<TbsCModel> listTbsCModel = null;
		StringBuilder center = new StringBuilder();

		if (pageParams.getSearchType() != null) {
			if (pageParams.getSearchType().equals("1")) { // 模糊搜索
				tbsCModel.getPageUtil().setLike(true);
				listTbsCModel = tbsCService.selectByModel(tbsCModel);
				center.append("{\"total\":\"" + tbsCModel.getPageUtil().getRowCount() + "\",\"rows\":"+str);
			} else {
				...
				if (sb.length() > 0) {
					...
					tbsCModel.getPageUtil().setAndCondition(sb.toString());
					listTbsCModel = tbsCService.selectByModel(tbsCModel);
					center.append("{\"total\":\"" + tbsCModel.getPageUtil().getRowCount() + "\",\"rows\":"+str);
				}
			}
			...
			if (pageParams.getGridName() == null) {
				listTbsCModel = tbsCService.selectByModel(tbsCModel);
				center.append("{\"total\":\"" + tbsCModel.getPageUtil().getRowCount() + "\",\"rows\":");
				suffix = "}";
			} else {
			}
		}
		...
		if (pageParams.getGridName() == null) { //datagrid
			center.append(JSON.toJSONString(listTbsCModel));
		} else {
		}
	...
	public void tbsCExport(HttpServletResponse response,TbsCModel tbsCModel) {
		List<TbsCModel> listTbsCModel=null;
		try {
			listTbsCModel=tbsCService.selectByModel(tbsCModel);
		} catch (Exception e1) {
		...
		Map<String, List<TbsCModel>> exportMap = new HashMap<String, List<TbsCModel>>();
		exportMap.put("sheet", listTbsCModel);
	...
	public void tbsCImport(HttpServletRequest request, HttpServletResponse response){
		...
		for (Map.Entry<String, MultipartFile> entity : fileMap.entrySet()) {
			...
			try {
				Map<String, List<TbsCModel>> ml=excelUtil.readExcel(new ByteArrayInputStream(mf.getBytes()), columnName, 2, TbsCModel.class);
				for(Map.Entry<String, List<TbsCModel>> map:ml.entrySet()){
					List<TbsCModel> lt=map.getValue();
					for(int i=0;i<lt.size();i++){
						try {
							tbsCService.insert(lt.get(i));
						} catch (Exception e) {
							e.printStackTrace();
						}
					}
     ...
     * 
     * <br>
     * <b>功能：</b>增加 TbsCModel<br>
     * <b>作者：</b> <br>
     * <b>日期：</b> 2012-10-11 <br>
     * @return
     */
	@RequestMapping("add.html")
	public void add(TbsCModel tbsCModel,HttpServletResponse response){
	}

    /**
     * 
     * <br>
     * <b>功能：</b>保存 TbsCModel<br>
     * <b>作者：</b> <br>
     * <b>日期：</b> 2012-10-11 <br>
     * @return
     */
	@RequestMapping("save.html") 
	public void save(TbsCModel tbsCModel,HttpServletResponse response){
		try{
		}catch(Exception e){
			util.toJsonMsg(response, 1, null);
			e.printStackTrace();
	...
	public void del(String[] ids,HttpServletResponse response){
		...
		try{
			 if(null!=ids && ids.length>0){
				 if(tbsCService.deleteByPrimaryKeys(ids)>0){
					 util.toJsonMsg(response, 0, null);
					 return;
				 }
			 }

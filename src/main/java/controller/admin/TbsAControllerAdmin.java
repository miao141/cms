...
@Controller
@RequestMapping("/admin/TbsAa/")
public class TbsAControllerAdmin extends BaseController{	private final static Logger log= Logger.getLogger(TbsAControllerAdmin.class);
	...
	public void chartsJson(HttpServletResponse response,String startTime,String endTime,String chartsName,Integer type) {
		...
		try {
			List<Map<?, ?>> list = tbsAService.charts(paramMap);
   ...
   /**
	...
	public String data(PageParams pageParams, TbsAModel tbsAModel) throws Exception {
		System.out.println("pageParams:" + pageParams + "|tbsAModel:" + tbsAModel);
		tbsAModel.getPageUtil().setPaging(true);
		String result = "[]";
		if (pageParams.getPage() != null) {
			try {
				tbsAModel.getPageUtil().setPageId(Integer.parseInt(pageParams.getPage())); // 当前页
			} catch (Exception e) {
				log.error(e);
			}
		}
		if (pageParams.getRows() != null) {
			try {
				tbsAModel.getPageUtil().setPageSize(Integer.parseInt(pageParams.getRows()));// 显示X条
			} catch (Exception e) {
				log.error(e);
			}
		}
		if (pageParams.getSort() != null) {
			try {
				tbsAModel.getPageUtil().setOrderByCondition(pageParams.getSort()); // 排序字段名称
			} catch (Exception e) {
				log.error(e);
			}
		}

		// tbsAModel.getPageUtil().setOrderDirection(true); //true false 表示 正序与倒序
        String str="";
        String suffix = "}";
		...
		if (pageParams.getSearchType() != null) {
			...
			if (pageParams.getGridName() == null) {
				listTbsAModel = tbsAService.selectByModel(tbsAModel);
				center.append("{\"total\":\"" + tbsAModel.getPageUtil().getRowCount() + "\",\"rows\":");
				suffix = "}";
			} else {
			}
		}
	...
	public void tbsAExport(HttpServletResponse response,TbsAModel tbsAModel) {
		List<TbsAModel> listTbsAModel=null;
		try {
			listTbsAModel=tbsAService.selectByModel(tbsAModel);
		} catch (Exception e1) {
		...
		Map<String, List<TbsAModel>> exportMap = new HashMap<String, List<TbsAModel>>();
		exportMap.put("sheet", listTbsAModel);
	...
	public void tbsAImport(HttpServletRequest request, HttpServletResponse response){
		...
		for (Map.Entry<String, MultipartFile> entity : fileMap.entrySet()) {
			...
			try {
				Map<String, List<TbsAModel>> ml=excelUtil.readExcel(new ByteArrayInputStream(mf.getBytes()), columnName, 2, TbsAModel.class);
				for(Map.Entry<String, List<TbsAModel>> map:ml.entrySet()){
					List<TbsAModel> lt=map.getValue();
					for(int i=0;i<lt.size();i++){
						try {
							tbsAService.insert(lt.get(i));
						} catch (Exception e) {
							e.printStackTrace();
						}
					}
	...
	public void add(TbsAModel tbsAModel,HttpServletResponse response){
		tbsAModel.setId(util.getUid());//设置主键
		System.out.println("tbsAModel:"+tbsAModel.toString());
	    try {

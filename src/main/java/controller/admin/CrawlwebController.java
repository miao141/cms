...
@Controller
@RequestMapping("/admin/Crawlweb/")
public class CrawlwebController extends BaseController{
	private final static Logger log= Logger.getLogger(CrawlwebController.class);
	private static  MethodUtil util = new MethodUtil();
	
	@Autowired
	private CrawlwebService<CrawlwebModel> crawlwebService; 
	...
	return new ModelAndView("admin/Crawlweb/index", modelMap);
 }
 
 
 @RequestMapping("data.html")
 @ResponseBody
 public String data(PageParams pageParams, CrawlwebModel crawlwebModel) throws Exception {
   crawlwebModel.getPageUtil().setPaging(true);
 	 String result = "[]";
 	 if (pageParams.getPage() != null) {
 		try {
 			crawlwebModel.getPageUtil().setPageId(Integer.parseInt(pageParams.getPage())); // ��ǰҳ
 		} catch (Exception e) {
 			log.error(e);
 		}
 	 }
   if (pageParams.getRows() != null) {
 		try {
 			crawlwebModel.getPageUtil().setPageSize(Integer.parseInt(pageParams.getRows()));// ��ʾX��
 		} catch (Exception e) {
			log.error(e);
		}
    }
	 if (pageParams.getSort() != null) {
	  try {
 		 crawlwebModel.getPageUtil().setOrderByCondition(pageParams.getSort()); // �����ֶ�����
 		} catch (Exception e) {
 			log.error(e);
 	    }
 	 }
 	 ...
 	 List<CrawlwebModel> listCrawlwebModel = null;
 	 StringBuilder center = new StringBuilder();
 	 
 	 if (pageParams.getSearchType() != null) {
 	 	if (pageParams.getSearchType().equals("1")) { // ģ������
 	 		crawlwebModel.getPageUtil().setLike(true);
 	 		listCrawlwebModel = crawlwebService.selectByModel(crawlwebModel);
    center.append("{\"total\":\""+ crawlwebModel.getPageUtil().getRowCount() +"\",\"rows\":"+str);
    } else {
    ...
    for (int i = 0; i < searchColumnNameArray.length; i++) {
 	  ...
 	  crawlwebModel.getPageUtil().setAndCondition(sb.toString());
 	  listCrawlwebModel = crawlwebService.selectByModel(crawlwebModel);
    center.append("{\"total\":\""+ crawlwebModel.getPageUtil().getRowCount() +"\",\"rows\":"+str);
 	  }
 	}
    ...
    center.append("{\"total\":\""+ crawlwebModel.getPageUtil().getRowCount() +"\",\"rows\":"+str);
 	...
 	if (pageParams.getGridName() == null) { //datagrid
 		center.append(JSON.toJSONString(listCrawlwebModel));
 		} else {
 		}
	...
	public String charts(){		
 	...
 	public String baseDlg(){
 		return "admin/Crawlweb/baseDlg";
 	}
 	
 	
 	@RequestMapping("importDlg.html")
 	public String importDlg(){
 		return "admin/Crawlweb/importDlg";
 	}
 	
 	
 	@RequestMapping("add.html")
 	public void add(CrawlwebModel crawlwebModel,HttpServletResponse response){
 	//crawlwebModel.setId(Integer.parseInt(util.getUid()));//��������
 		if(StringUtils.isEmpty(crawlwebModel.getCreate_time())){
 			//String str = DateUtil.formatDate(date, pattern)getFomartDate();
 			String str = DateUtil.getFomartDate(new Date(),"yyyy-MM-dd");
 			crawlwebModel.setCreate_time(str);
 		}
 	System.out.println("tbsBModel:"+crawlwebModel.toString());
 	 try {
 		if(crawlwebService.insert(crawlwebModel)>0){
 	...
 	public void save(CrawlwebModel crawlwebModel,HttpServletResponse response){
 		try{
 			if(crawlwebService.updateByPrimaryKey(crawlwebModel)>0){
 				 util.toJsonMsg(response, 0, null);
 	  			 return;
 	 		}
 		}catch(Exception e){
 	...
 	public void del(String[] ids,HttpServletResponse response){
 		...
 		if(crawlwebService.deleteByPrimaryKeys(ids)>0){
 		util.toJsonMsg(response, 0, null);
 		return;
 	 	}
 	 	}

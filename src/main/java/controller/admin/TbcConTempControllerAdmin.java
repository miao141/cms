...
public class TbcConTempControllerAdmin extends BaseController{	private final static Logger log= Logger.getLogger(TbcConTempControllerAdmin.class);
	...
	private TbcConTempService<TbcConTempModel> tbcConTempService; 
	...
	public ModelAndView index(String id, ModelMap modelMap, HttpServletRequest request) {
		...
		return new ModelAndView("admin/TbcConTemp/index", modelMap);
	}
	...
	public String charts(){
		return "admin/TbcConTemp/charts";
	}
	...
	public void chartsJson(HttpServletResponse response,String startTime,String endTime,String chartsName,Integer type) {
		...
		try {
			List<Map<?, ?>> list = tbcConTempService.charts(paramMap);
	...
	public String baseDlg(){
		return "admin/TbcConTemp/baseDlg";
	}
	...
	public String importDlg(){
		return "admin/TbcConTemp/importDlg";
	}
	...
	public String searchDlg(){
		return "admin/TbcConTemp/searchDlg";
	}
   ...
   /**
	...
	@ResponseBody
	public String data(PageParams pageParams, TbcConTempModel tbcConTempModel) throws Exception {
		System.out.println("pageParams:" + pageParams + "|tbcConTempModel:" + tbcConTempModel);
		tbcConTempModel.getPageUtil().setPaging(true);
		String result = "[]";
		if (pageParams.getPage() != null) {
			try {
				tbcConTempModel.getPageUtil().setPageId(Integer.parseInt(pageParams.getPage())); // 当前页
			} catch (Exception e) {
				log.error(e);
			}
		}
		if (pageParams.getRows() != null) {
			try {
				tbcConTempModel.getPageUtil().setPageSize(Integer.parseInt(pageParams.getRows()));// 显示X条
			} catch (Exception e) {
				log.error(e);
			}
		}
		if (pageParams.getSort() != null) {
			try {
				tbcConTempModel.getPageUtil().setOrderByCondition(pageParams.getSort()); // 排序字段名称
			} catch (Exception e) {
				log.error(e);
			}
		}
		...
		List<TbcConTempModel> listTbcConTempModel = null;
		StringBuilder center = new StringBuilder();

		if (pageParams.getSearchType() != null) {
			if (pageParams.getSearchType().equals("1")) { // 模糊搜索
				tbcConTempModel.getPageUtil().setLike(true);
				listTbcConTempModel = tbcConTempService.selectByModel(tbcConTempModel);
				center.append("{\"total\":\"" + tbcConTempModel.getPageUtil().getRowCount() + "\",\"rows\":"+str);
			} else {
				...
				if (sb.length() > 0) {
					...
					tbcConTempModel.getPageUtil().setAndCondition(sb.toString());
					listTbcConTempModel = tbcConTempService.selectByModel(tbcConTempModel);
					center.append("{\"total\":\"" + tbcConTempModel.getPageUtil().getRowCount() + "\",\"rows\":"+str);
				}
			}
			...
			if (pageParams.getGridName() == null) {
				listTbcConTempModel = tbcConTempService.selectByModel(tbcConTempModel);
				center.append("{\"total\":\"" + tbcConTempModel.getPageUtil().getRowCount() + "\",\"rows\":");
				suffix = "}";
			} else {
			}
		}
		...
		if (pageParams.getGridName() == null) { //datagrid
			center.append(JSON.toJSONString(listTbcConTempModel));
		} else {
		}
	...
	public void tbcConTempExport(HttpServletResponse response,TbcConTempModel tbcConTempModel) {
		List<TbcConTempModel> listTbcConTempModel=null;
		try {
			listTbcConTempModel=tbcConTempService.selectByModel(tbcConTempModel);
		} catch (Exception e1) {
		...
		Map<String, List<TbcConTempModel>> exportMap = new HashMap<String, List<TbcConTempModel>>();
		exportMap.put("sheet", listTbcConTempModel);
	...
	public void tbcConTempImport(HttpServletRequest request, HttpServletResponse response){
		...
		for (Map.Entry<String, MultipartFile> entity : fileMap.entrySet()) {
			...
			try {
				Map<String, List<TbcConTempModel>> ml=excelUtil.readExcel(new ByteArrayInputStream(mf.getBytes()), columnName, 2, TbcConTempModel.class);
				for(Map.Entry<String, List<TbcConTempModel>> map:ml.entrySet()){
					List<TbcConTempModel> lt=map.getValue();
					for(int i=0;i<lt.size();i++){
						try {
							tbcConTempService.insert(lt.get(i));
						} catch (Exception e) {
							e.printStackTrace();
						}
					}
     ...
     * 
     * <br>
     * <b>功能：</b>增加 TbcConTempModel<br>
     * <b>作者：</b> <br>
     * <b>日期：</b> 2012-10-11 <br>
     * @return
     */
	@RequestMapping("add.html")
	public void add(TbcConTempModel tbcConTempModel,HttpServletResponse response){
		tbcConTempModel.setId(util.getUid());//设置主键
		System.out.println("tbcConTempModel:"+tbcConTempModel.toString());
	    try {
			if(tbcConTempService.insert(tbcConTempModel)>0){
				util.toJsonMsg(response, 0, null);
				return;
			};
		} catch (Exception e) {
     ...
     * 
     * <br>
     * <b>功能：</b>保存 TbcConTempModel<br>
     * <b>作者：</b> <br>
     * <b>日期：</b> 2012-10-11 <br>
     * @return
     */
	@RequestMapping("save.html") 
	public void save(TbcConTempModel tbcConTempModel,HttpServletResponse response){
		try{
		 	if(tbcConTempService.updateByPrimaryKey(tbcConTempModel)>0){
			    util.toJsonMsg(response, 0, null);
			    return;
			 }
		}catch(Exception e){

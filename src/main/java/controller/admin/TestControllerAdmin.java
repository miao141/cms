...
public class TestControllerAdmin extends BaseController{	private final static Logger log= Logger.getLogger(TestControllerAdmin.class);
	...
	private TestService<TestModel> testService; 
	...
	public ModelAndView index(String id, ModelMap modelMap, HttpServletRequest request) {
		...
		return new ModelAndView("admin/Test/index", modelMap);
	}
	...
	public void chartsJson(HttpServletResponse response,String startTime,String endTime,String chartsName,Integer type) {
		...
		try {
			List<Map<?, ?>> list = testService.charts(paramMap);
   ...
   /**
	...
	@ResponseBody
	public String data(PageParams pageParams, TestModel testModel) throws Exception {
	...
	public void testExport(HttpServletResponse response,TestModel testModel) {
		List<TestModel> listTestModel=null;
		try {
			listTestModel=testService.selectByModel(testModel);
		} catch (Exception e1) {
		...
		Map<String, List<TestModel>> exportMap = new HashMap<String, List<TestModel>>();
	...
	public void testImport(HttpServletRequest request, HttpServletResponse response){
		...
		for (Map.Entry<String, MultipartFile> entity : fileMap.entrySet()) {
			...
			try {
				Map<String, List<TestModel>> ml=excelUtil.readExcel(new ByteArrayInputStream(mf.getBytes()), columnName, 2, TestModel.class);
				for(Map.Entry<String, List<TestModel>> map:ml.entrySet()){
					List<TestModel> lt=map.getValue();
					for(int i=0;i<lt.size();i++){
						try {
							testService.insert(lt.get(i));
						} catch (Exception e) {
							e.printStackTrace();
						}
					}
	...
	public void add(TestModel testModel,HttpServletResponse response){
		testModel.setId(util.getUid());//设置主键
		System.out.println("testModel:"+testModel.toString());
	    try {
			if(testService.insert(testModel)>0){
				util.toJsonMsg(response, 0, null);
				return;
			};
		} catch (Exception e) {
	...
	public void save(TestModel testModel,HttpServletResponse response){
		try{
		 	if(testService.updateByPrimaryKey(testModel)>0){
			    util.toJsonMsg(response, 0, null);
			    return;
			 }
		}catch(Exception e){
	...
	public void del(String[] ids,HttpServletResponse response){
		...
		try{
			 if(null!=ids && ids.length>0){
				 if(testService.deleteByPrimaryKeys(ids)>0){
					 util.toJsonMsg(response, 0, null);
					 return;
				 }
			 }

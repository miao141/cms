...
@Controller
@RequestMapping("/admin/TbcMenu/")
public class TbcMenuControllerAdmin extends BaseController{	private final static Logger log= Logger.getLogger(TbcMenuControllerAdmin.class);
	...
	private TbcMenuService<TbcMenuModel> tbcMenuService; 
	...
	public ModelAndView index(String id, ModelMap modelMap, HttpServletRequest request) {
		...
		return new ModelAndView("admin/TbcMenu/index", modelMap);
	}
	...
	public void chartsJson(HttpServletResponse response,String startTime,String endTime,String chartsName,Integer type) {
		...
		try {
			List<Map<?, ?>> list = tbcMenuService.charts(paramMap);
	...
	public String importDlg(){
		return "admin/TbcMenu/importDlg";
	}
	...
	public String searchDlg(){
		return "admin/TbcMenu/searchDlg";
	}
   ...
   /**
	...
	@ResponseBody
	public String data(PageParams pageParams, TbcMenuModel tbcMenuModel) throws Exception {
		System.out.println("pageParams:" + pageParams + "|tbcMenuModel:" + tbcMenuModel);
		tbcMenuModel.getPageUtil().setPaging(true);
		String result = "[]";
		if (pageParams.getPage() != null) {
			try {
				tbcMenuModel.getPageUtil().setPageId(Integer.parseInt(pageParams.getPage())); // 当前页
			} catch (Exception e) {
				log.error(e);
			}
		}
		if (pageParams.getRows() != null) {
			try {
				tbcMenuModel.getPageUtil().setPageSize(Integer.parseInt(pageParams.getRows()));// 显示X条
			} catch (Exception e) {
				log.error(e);
			}
		}
		if (pageParams.getSort() != null) {
			try {
				tbcMenuModel.getPageUtil().setOrderByCondition(pageParams.getSort()); // 排序字段名称
			} catch (Exception e) {
				log.error(e);
			}
		}

		// tbcMenuModel.getPageUtil().setOrderDirection(true); //true false 表示 正序与倒序
        String str="";
        String suffix = "}";
		...
		List<TbcMenuModel> listTbcMenuModel = null;
		StringBuilder center = new StringBuilder();

		if (pageParams.getSearchType() != null) {
			if (pageParams.getSearchType().equals("1")) { // 模糊搜索
				tbcMenuModel.getPageUtil().setLike(true);
				listTbcMenuModel = tbcMenuService.selectByModel(tbcMenuModel);
				center.append("{\"total\":\"" + tbcMenuModel.getPageUtil().getRowCount() + "\",\"rows\":"+str);
			} else {
				...
				if (sb.length() > 0) {
					...
					tbcMenuModel.getPageUtil().setAndCondition(sb.toString());
					listTbcMenuModel = tbcMenuService.selectByModel(tbcMenuModel);
					center.append("{\"total\":\"" + tbcMenuModel.getPageUtil().getRowCount() + "\",\"rows\":"+str);
				}
			}
			...
			if (pageParams.getGridName() == null) {
				listTbcMenuModel = tbcMenuService.selectByModel(tbcMenuModel);
				center.append("{\"total\":\"" + tbcMenuModel.getPageUtil().getRowCount() + "\",\"rows\":");
				suffix = "}";
			} else {
				if (tbcMenuModel.getId() == null) { // 首次打开
					tbcMenuModel.getPageUtil().setAndCondition("parentId IS NULL");
					tbcMenuModel.getPageUtil().setOrderByCondition("sortNumber");
					listTbcMenuModel = tbcMenuService.selectByModel(tbcMenuModel);
					center.append("{\"total\":\"" + tbcMenuModel.getPageUtil().getRowCount() + "\",\"rows\":"+str);
				} else { // 子节点加载
					tbcMenuModel.setParentId(tbcMenuModel.getId());
		...
		if (pageParams.getGridName() == null) { //datagrid
			center.append(JSON.toJSONString(listTbcMenuModel));
		} else {
			if (listTbcMenuModel.size() > 0) { // TreeGrid
				for (int i = 0; i < listTbcMenuModel.size(); i++) {
					TbcMenuModel model = listTbcMenuModel.get(i);
					String json = JSON.toJSONString(model);
					TbcMenuModel model2 = new TbcMenuModel();
					model2.setParentId(model.getId());
					if (tbcMenuService.selectByModelCount(model2) > 0) {
	...
	public void tbcMenuExport(HttpServletResponse response,TbcMenuModel tbcMenuModel) {
		List<TbcMenuModel> listTbcMenuModel=null;
		try {
			listTbcMenuModel=tbcMenuService.selectByModel(tbcMenuModel);
		} catch (Exception e1) {
		...
		Map<String, List<TbcMenuModel>> exportMap = new HashMap<String, List<TbcMenuModel>>();
		exportMap.put("sheet", listTbcMenuModel);
	...
	public void tbcMenuImport(HttpServletRequest request, HttpServletResponse response){
		...
		for (Map.Entry<String, MultipartFile> entity : fileMap.entrySet()) {
			...
			try {
				Map<String, List<TbcMenuModel>> ml=excelUtil.readExcel(new ByteArrayInputStream(mf.getBytes()), columnName, 2, TbcMenuModel.class);
				for(Map.Entry<String, List<TbcMenuModel>> map:ml.entrySet()){
					List<TbcMenuModel> lt=map.getValue();
					for(int i=0;i<lt.size();i++){
						try {
							tbcMenuService.insert(lt.get(i));
						} catch (Exception e) {
							e.printStackTrace();
						}
					}
     ...
     * 
     * <br>
     * <b>功能：</b>增加 TbcMenuModel<br>
     * <b>作者：</b> <br>
     * <b>日期：</b> 2012-10-11 <br>
     * @return
     */
	@RequestMapping("add.html")
	public void add(TbcMenuModel tbcMenuModel,HttpServletResponse response){
		tbcMenuModel.setId(util.getUid());//设置主键
		System.out.println("tbcMenuModel:"+tbcMenuModel.toString());
	    try {
			if(tbcMenuService.insert(tbcMenuModel)>0){
				util.toJsonMsg(response, 0, null);
				return;
			};
		} catch (Exception e) {
     ...
     * 
     * <br>
     * <b>功能：</b>保存 TbcMenuModel<br>
     * <b>作者：</b> <br>
     * <b>日期：</b> 2012-10-11 <br>
     * @return
     */
	@RequestMapping("save.html") 
	public void save(TbcMenuModel tbcMenuModel,HttpServletResponse response){
		try{
		 	if(tbcMenuService.updateByPrimaryKey(tbcMenuModel)>0){
			    util.toJsonMsg(response, 0, null);
			    return;
			 }
		}catch(Exception e){
	...
	public void del(String[] ids,HttpServletResponse response){
		...
		try{
			 if(null!=ids && ids.length>0){
				 if(tbcMenuService.deleteByPrimaryKeys(ids)>0){
					 util.toJsonMsg(response, 0, null);
					 return;
				 }
			 }
